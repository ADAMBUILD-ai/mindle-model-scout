...
def filter_candidates(candidates: list[dict[str, Any]], profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for candidate in candidates:
        if profile.get("task_hint") and candidate.get("resource_type", "model") == "model" and candidate.get("pipeline_tag") != profile["task_hint"]:
            continue
        ...
        if int(candidate.get("likes") or 0) < int(profile.get("min_likes") or 0):
            continue
        if profile.get("library_hint") and candidate.get("library_name") != profile["library_hint"]:
            continue
